...
// function for focusing at a distance from the camera2
THREE.CinematicCamera.prototype.focusAt = function ( focusDistance ) {
	...
	// distance from the camera2 (normal to frustrum) to focus on
	this.focus = focusDistance;

	// the nearest point from the camera2 which is in focus (unused)
	this.nearPoint = ( this.hyperFocal * this.focus ) / ( this.hyperFocal + ( this.focus - focalLength ) );

	// the farthest point from the camera2 which is in focus (unused)
...
THREE.CinematicCamera.prototype.initPostProcessing = function () {
	...
	if ( this.postprocessing.enabled ) {
		...
		this.postprocessing.scene2 = new THREE.Scene();

		this.postprocessing.camera2 = new THREE.OrthographicCamera( window.innerWidth / - 2, window.innerWidth / 2,	window.innerHeight / 2, window.innerHeight / - 2, - 10000, 10000 );
...
THREE.CinematicCamera.prototype.renderCinematic = function ( scene, renderer ) {
	...
	if ( this.postprocessing.enabled ) {
		...
		// Render scene2 into texture

		scene.overrideMaterial = null;
		renderer.render( scene, camera2, this.postprocessing.rtTextureColor, true );

		// Render depth into texture

		scene.overrideMaterial = this.materialDepth;
		renderer.render( scene, camera2, this.postprocessing.rtTextureDepth, true );
