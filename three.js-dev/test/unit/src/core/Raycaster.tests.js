...
function checkRayDirectionAgainstReferenceVector( rayDirection, refVector, assert ) {
	...
	assert.ok( refVector.x - rayDirection.x <= Number.EPSILON && refVector.y - rayDirection.y <= Number.EPSILON && refVector.z - rayDirection.z <= Number.EPSILON, "camera2 is pointing to" +
	...
	QUnit.module( 'Raycaster', () => {
		...
		QUnit.test( "setFromCamera (Perspective)", ( assert ) => {
			...
			assert.ok( rayDirection.x === 0 && rayDirection.y === 0 && rayDirection.z === - 1,
				"camera2 is looking straight to -z and so does the ray in the middle of the screen" );
