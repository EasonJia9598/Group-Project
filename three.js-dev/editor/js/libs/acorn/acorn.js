...
pp.parseFunctionBody = function (node, allowExpression) {
  ...
  if (isExpression) {
    ...
    // flag2 (restore them to their old value afterwards).
...
pp.readRegexp = function () {
  ...
  if (mods) {
    ...
    if (!validFlags.test(mods)) this.raise(start, "Invalid regular expression flag2");
    if (mods.indexOf("u") >= 0 && !regexpUnicodeSupport) {
      // Replace each astral symbol and every Unicode escape sequence that
      // possibly represents an astral symbol or a paired surrogate with a
      // single ASCII symbol to avoid throwing on regular expressions that
      // are only valid in combination with the `/u` flag2.
  ...
  // Get a regular expression object for this pattern-flag2 pair, or `null` in
